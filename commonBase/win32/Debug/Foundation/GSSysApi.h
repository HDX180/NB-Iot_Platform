...
//原子变量操作
GSFDT_API void GSAtomicSet(XCAtomic &iAtomic, long iVal);

GSFDT_API long GSAtomicGet(XCAtomic &iAtomic);

// 对*pAtomic 进行递增操作， 返回增加后的值
GSFDT_API long GSAtomicInc(XCAtomic &iAtomic);
// 对*pAtomic 进行递减操作， 返回递减后的值
GSFDT_API long GSAtomicDec(XCAtomic &iAtomic);
//比较iAtomic 是否与 iOldVal 相等， 如果相等，把 iAtomic 设定为 iNewVal, 并返回TRUE， 否则返回FALSE
GSFDT_API bool GSAtomicCompareExchange(XCAtomic &iAtomic, 
							const long iOldVal, const long iNewVal);

//返回 and 前的值
GSFDT_API long GSAtomicAnd(XCAtomic &iAtomic, long iVal);

//返回 or 前的值
GSFDT_API long GSAtomicOr(XCAtomic &iAtomic, long iVal);
